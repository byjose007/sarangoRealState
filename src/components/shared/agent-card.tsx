    ...
    <article className="group h-full overflow-hidden rounded-lg border border-border bg-card transition-shadow hover:shadow-lift">
      ...
      <div className="p-5">
        <h3 className="font-display text-lg tracking-tight">
          <Link href={`/agents/${agent.slug}`} className="inline-flex items-center gap-1.5 hover:text-primary">
            {agent.name}
        ...
        {agent.license && (
          <p className="mt-1 font-mono text-[0.68rem] font-medium text-brass">
            {agent.license}
          </p>
        )}
